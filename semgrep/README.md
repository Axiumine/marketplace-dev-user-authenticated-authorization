# Semgrep — marketplace-dev-user-authenticated-authorization
...
`marketplace-no-log-auth-token`, the latter directly relevant here since this
service is where access/refresh tokens are generated and rotated
...
- **Scope is the explicit file list, not `.semgrepignore`.** Because files are
  ...
  (This repo has no `.orig` merge leftovers under `src/` at the time of
  writing, but the `find` list has no `.orig` entry either way — one dropped
  in later would stay correctly out of scope.)
- **Parsed-lines ≈ 63.7%, not 100%, and that is expected.** The bypass parses each
