 ...
 *
 * ⚠️ **Nothing here configures TLS, and nothing here may.** Until the telemetry audit this file handed
 * `Sentry.init` a `transportOptions.httpModule` that turned certificate verification off on every
 * outbound request — unconditional on environment, in all nine services, and the only outbound HTTPS
 * these repos configure themselves. A collector behind a certificate this machine does not already
 * trust is reached by **trusting its CA**, from outside the process:
 *
 * ```bash
